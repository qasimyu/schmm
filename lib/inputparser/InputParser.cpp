...
void InputParser::parseArgs(int argc, char *argv[]) {
	...
	if(subcmd.compare("-h") == 0 || subcmd.compare("--help") == 0) {
		...
		cerr << "SCHMM version " << current_version << endl;
		exit(0);
	}
...
void InputParser::usage(const char* app) {
	cerr << "\nSCHMM version: " << current_version << endl;
	cerr << "Usage: " << app << " [subcommand] [options]" << endl
		...
		<< "    prepareinputs        fetch inputs from tumor SCS data for calling CNAs" << endl
		<< "    callcnas             detect copy number alterations in single cell" << endl
		<< endl
		<< "Author: Zhenhua Yu <qasim0208@163.com>\n" << endl;
}

void InputParser::usage_prepareInputs(const char* app) {
	cerr << "Usage: schmm " << app << " [options]" << endl
		<< endl
		<< "Options:" << endl
		<< "    -h, --help                      give this information" << endl
		...
		<< endl
		<< "Example:" << endl
		<< "    schmm " << app << " -b example.bam -r hg19.fasta -m hg19.bw -s hg19.snp151.txt -M -w 50000 -o example.out" << endl
		<< endl
		<< "Author: Zhenhua Yu <qasim0208@163.com>\n" << endl;
}
...
void InputParser::usage_callCNAs(const char* app) {
	cerr << "Usage: schmm " << app << " [options]" << endl
		...
		<< endl
		<< "Example:" << endl
		<< "    schmm " << app << " -i example.out -d 10 -D 200 -C 5 -p 2 -t 5 -o results" << endl
		<< endl
		<< "Author: Zhenhua Yu <qasim0208@163.com>\n" << endl;
}
